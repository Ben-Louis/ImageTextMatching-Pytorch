...
class CircleLoss(nn.Module):
    def __init__(self, margin: float, scale: float, **kwargs) -> None:
    ...
    def forward(self, img_feature, cap_feature):
        feat = nn.functional.normalize(torch.cat((img_feature, cap_feature), dim=0))
        label = torch.arange(img_feature.size(0), device=feat.device, dtype=torch.int32)
